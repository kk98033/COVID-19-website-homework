    ...
    inp.addEventListener("input", function(e) {
        ...
        for (i = 0; i < arr.length; i++) {
          ...
          if (arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
            ...
            b.innerHTML = "<strong style='color: yellow;'>" + arr[i].substr(0, val.length) + "</strong>"; //make the matching letters bold:
...
function GetWorldContryName(url){
  return fetch(url, {})
  
  .then(res => res.json())
  .then(data => {
      var countries = OrganizeData(data)
...
GetWorldContryName(url).then((data) => {
  countries = data
  // console.log(countries);
